package com.movierecommender.statistics
...
object StatisticsRecommender {
  val MONGODB_RATIONG_COLLECTION="Rating"
  val MONGODB_MOVIE_COLLECTION="Movie"
  //统计的表的名称
  val RATE_MORE_MOVIES="RateMoreMovies"
  val RATE_MORD_RECENTLY_MOVIES="RateMoreRecentlyMovies"
  ...
  def main(args: Array[String]): Unit = {
    ...
    //数据加载进来,把数据从MongoDB中拿到ratingDF中
    val ratingDF=spark
      .read
      .option("uri",mongoConfig.uri)
      .option("collection",MONGODB_RATIONG_COLLECTION)
    ...
    rateMoreRecentlyMovies
      .write
      .option("uri",mongoConfig.uri)
      .option("collection",RATE_MORD_RECENTLY_MOVIES)
      .mode("overwrite")
      .format("com.mongodb.spark.sql")
      .save()


    //3.优质电影统计，统计每个电影的平均评分
    ...
    //4.各类别电影Top统计，统计每种电影类型中评分最高的10个电影
