package com.movierecommender.streaming
...
object ConnHelper extends Serializable{
  lazy val jedis=new Jedis("192.168.1.101:6379")
  lazy val mongoClient=MongoClient(MongoClientURI("mongodb://192.168.1.101:27017/recommender"))
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //广播电影相似度矩阵
    //把数据从MongoDB中读取出来，转换成为Map[Int,Map[Int,Double]]
    ...
    val ratingStream=kafkaStream.map{
      case msg=>val attr=msg.value().split("\\|")
      (attr(0).toInt,attr(1).toInt,attr(2).toDouble,attr(3).toInt)
    }
    //核心实时推荐算法
    ratingStream.foreachRDD{
      rdd=>rdd.map{
        case (uid,mid,score,timestamp)=>println(">>>>>>>>>>>>>>>>>>")
          //获取当前最近的M次电影评分
  ...
  def getUserRecentlyRating(num:Int,uid:Int,jedis:Jedis):Array[(Int,Double)]={
    println("用户最近评分")
    //从用户的队列中取出num个评分
    jedis.lrange("uid:"+uid.toString,0,num-1)
      .map{
  ...
  def getTopSimMovies(num:Int,mid:Int,uid:Int,simMovies:scala.collection.Map
    [Int,scala.collection.immutable.Map[Int,Double]])
  ...
  def computeMovieScores(
    ...
    score.groupBy(_._1).map{
      case (mid,sims)=>(mid,sims.map(_._2).sum/sims.length+log(increMap.getOrDefault(mid,1))-log(decreMap.getOrDefault(mid,1)))
